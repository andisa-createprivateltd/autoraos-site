...
import { BRANDS, CREATEPRIVATE_WHATSAPP, PROVINCES } from "@/lib/constants";
...
export function BookingForm({ prefill }: { prefill?: PrefillData }) {
  ...
  const summaryText = useMemo(() => {
    if (!form.dealershipName) return "Book your 15-minute audit";
    return `Book an Audit for ${form.dealershipName}`;
  }, [form.dealershipName]);
  ...
  if (success) {
    return (
      <div className="rounded-3xl border border-green-700/20 bg-green-50 p-6">
        ...
        <h3 className="mt-2 text-xl font-semibold text-coal">Your Dealer Lead Audit is booked.</h3>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 rounded-3xl border border-steel/15 bg-white p-6 shadow-soft">
      <div>
        <p className="text-xs uppercase tracking-[0.22em] text-tide">Dealer Lead Audit</p>
        <h2 className="mt-2 text-2xl font-semibold text-coal">{summaryText}</h2>
        <p className="mt-2 text-sm text-steel">Choose an available 15-minute slot and share your dealership details.</p>
      </div>
      ...
      >
        {isSubmitting ? "Booking..." : "Confirm 15-Minute Audit"}
      </button>

      <p className="text-xs text-steel">
        Optional WhatsApp confirmation available after submission. For direct chat now, use {" "}
        <a
          href={waLink(CREATEPRIVATE_WHATSAPP, "Hi CreatePrivateLtd, I need help with lead generation.")}
          target="_blank"
          rel="noreferrer"
