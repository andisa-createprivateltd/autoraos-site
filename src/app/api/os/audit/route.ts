...
import { getCanonicalAuditFeed, resolveDashboardScope } from "@/lib/autora-dashboard";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const payload = await getCanonicalAuditFeed(
      auth.session,
      scope,
      {
        sla: searchParams.get("action") || searchParams.get("sla") || undefined,
    ...
    console.error("OS audit API failed", error);
